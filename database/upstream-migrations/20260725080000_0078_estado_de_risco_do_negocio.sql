-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260725080000_0078_estado_de_risco_do_negocio.sql. Do not edit.
...
comment on table public.crm_lead_risk_states is
  'Estado de risco por negócio (wave 7 — o ciclo). FORA de crm_leads pelos motivos da 0075 (pulso que mente, 409 fantasma), mas DENTRO da publicação crm_realtime, ao contrário de crm_lead_scores: risco é mudança de estado, não telemetria. O escritor só grava quando o bucket muda.';
...
begin
  if not exists (
    ...
    execute 'alter publication crm_realtime add table public.crm_lead_risk_states';
  end if;
end $$;
