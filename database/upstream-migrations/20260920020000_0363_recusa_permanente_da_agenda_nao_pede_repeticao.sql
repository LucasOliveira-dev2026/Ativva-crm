-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260920020000_0363_recusa_permanente_da_agenda_nao_pede_repeticao.sql. Do not edit.
...
begin
 if fortis.current_user_id() is null or not public.fn_role_at_least(p_org,'agent') or not public.fn_support_write_allowed(p_org) then raise exception 'meet_forbidden' using errcode='42501';end if;
 ...
 if not found or a.owner_user_id is distinct from fortis.current_user_id() or not exists(select 1 from public.user_organizations where organization_id=p_org and user_id=fortis.current_user_id() and revoked_at is null) then raise exception 'meet_forbidden' using errcode='42501';end if;
