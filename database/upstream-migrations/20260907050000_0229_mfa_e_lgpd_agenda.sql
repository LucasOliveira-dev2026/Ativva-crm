-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260907050000_0229_mfa_e_lgpd_agenda.sql. Do not edit.
...
begin
 ...
 if fortis.current_user_id() is not null and not public.fn_session_mfa_proven() then raise exception 'appointment_mfa_required' using errcode='42501';end if;
...
returns jsonb language plpgsql security definer set search_path=public as $$
begin
 if fortis.current_user_id() is null or not public.fn_role_at_least(p_org,'manager') or not public.fn_support_write_allowed(p_org) then raise exception 'agenda_settings_forbidden' using errcode='42501'; end if;
...
begin
 ...
 if fortis.current_user_id() is null or not public.fn_support_write_allowed(p_organization_id)
