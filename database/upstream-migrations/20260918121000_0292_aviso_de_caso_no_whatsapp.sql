-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260918121000_0292_aviso_de_caso_no_whatsapp.sql. Do not edit.
-- ═══════════════════════════════════════════════════════════════════════════
...
create table if not exists public.config_aviso_de_caso (
  ...
  criado_por          uuid references identity.users(id) on delete set null,
  atualizado_por      uuid references identity.users(id) on delete set null,
...
declare
  ...
  v_variantes text[];
begin
  -- Papel + suporte, nesta ordem e na MESMA transação da escrita. `fortis.current_user_id()`
  -- nulo é o caminho do service role: quem escreve configuração é gente.
  if fortis.current_user_id() is null or p_org is null
  ...
  values
    (p_org, p_channel, p_telefone, nullif(btrim(p_rotulo), ''), coalesce(p_ligado, false), fortis.current_user_id(), fortis.current_user_id())
  on conflict (organization_id) do update
    set channel_session_id = excluded.channel_session_id,
        ...
        atualizado_por     = fortis.current_user_id(),
