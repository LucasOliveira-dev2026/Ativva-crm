-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260811100000_0149_definer_valida_membership.sql. Do not edit.
...
--
--     if fortis.current_user_id() is not null and not fn_role_at_least(org, '<papel>') then
--       raise exception ...
--
-- O `fortis.current_user_id() is not null` é a parte que faz isto ser seguro SEM quebrar o
-- produto: worker, cron e trigger chamam com `service_role` (ou como `postgres`
-- dentro de um trigger), onde não há JWT e `fortis.current_user_id()` é null — esses passam
...
begin
  ...
  -- (service_role ou postgres, sem JWT) têm fortis.current_user_id() null e seguem passando.
...
comment on function public.retrieve_top_k_chunks(uuid, uuid, public.vector, integer, real) is
  'Busca vetorial na KB de uma organização. Valida membership do chamador quando há sessão de usuário (fortis.current_user_id() não nulo); worker/cron com crm_service passam direto.';
