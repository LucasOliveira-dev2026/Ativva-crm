-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260920002100_0351_comanda_e_financeiro.sql. Do not edit.
...
create table if not exists public.sales (
  ...
  attendant_user_id uuid references identity.users(id) on delete set null,
...
create table if not exists public.commission_rules (
  ...
  attendant_user_id uuid references identity.users(id) on delete cascade,
...
create table if not exists public.commissions (
  ...
  attendant_user_id uuid not null references identity.users(id) on delete restrict,
...
declare
  ...
  v_entry      uuid;
begin
  if fortis.current_user_id() is null or not public.fn_role_at_least(p_org, 'agent') then
  ...
  values (
    ...
    v_sale.currency, format('Comanda #%s', v_sale.number), 'paid', now(), 'sale', fortis.current_user_id()
  )
  ...
  if p_loyalty_points > 0 and v_sale.contact_id is not null then
    ...
    values (
      ...
      format('sale:%s', p_sale), fortis.current_user_id()
    )
...
declare
  ...
  v_novo   uuid;
begin
  if fortis.current_user_id() is null or not public.fn_role_at_least(p_org, 'manager') then
  ...
  loop
    ...
    values (
      ...
      v_orig.id, fortis.current_user_id()
    )
  ...
  select p_org, v_sale.contact_id, -l.points, 'Estorno da comanda', p_sale,
         format('reversal:%s', p_sale), fortis.current_user_id()
...
begin
  foreach t in array array['sales', 'sale_items', 'commission_rules', 'commissions',
    ...
    $f$, t, t);
    execute format('revoke all on public.%I from crm_anonymous', t);
  end loop;
end $$;
