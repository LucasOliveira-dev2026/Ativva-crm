-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260917120000_0271_extensoes_declarativas.sql. Do not edit.
...
create table if not exists public.extension_catalogs (
  ...
  admitted_by uuid references identity.users(id) on delete set null,
...
create table if not exists public.extension_installations (
  ...
  installed_by uuid references identity.users(id) on delete set null,
...
alter table public.extension_installations
  ...
  add column if not exists removed_by uuid references identity.users(id) on delete set null;
  ...
  for select to crm_authenticated using (
    ...
    and (
      exists (select 1 from public.user_organizations u where u.organization_id = organization_extensions.organization_id
        and u.user_id = fortis.current_user_id() and u.accepted_at is not null and u.revoked_at is null)
...
begin
  if p_actor is null or not exists (select 1 from identity.users where id = p_actor) then
