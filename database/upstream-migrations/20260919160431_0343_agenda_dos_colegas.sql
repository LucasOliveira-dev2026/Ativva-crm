-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260919160431_0343_agenda_dos_colegas.sql. Do not edit.
-- =============================================================================
...
--   * PESSOA (`fortis.current_user_id()` não nulo) com papel efetivo `agent`: recortada por
...
--     `fortis.current_user_id()` é nulo e a regra não se aplica — a reconciliação escreve o
--     que o Google mandou.
...
begin
 ...
 -- `fortis.current_user_id()` nulo é service role (workers, canal remoto do Google) e a regra
 -- não se aplica — está declarado no cabeçalho desta migration.
 if fortis.current_user_id() is not null and not public.fn_role_at_least(p_org,'manager')
    and not public.fn_colegas_podem_mexer_na_agenda(p_org)
    and a.owner_user_id is distinct from fortis.current_user_id() then
...
--
-- Pelo client DA SESSÃO no Server Action: é `fortis.current_user_id()` que faz a função
...
begin
 ...
 if fortis.current_user_id() is null
