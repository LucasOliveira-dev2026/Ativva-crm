-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260905210000_0220_suporte_temporario_por_sessao.sql. Do not edit.
...
-- RESTRICT impediria logout. O início valida identity.sessions sob lock.
...
returns jsonb language sql stable security definer set search_path = public as $f$
 ...
 when (p.mfa_required or exists(select 1 from identity.mfa_factors f where f.user_id=s.actor_user_id and f.status='verified'))
 and coalesce(fortis.current_aal(),'aal1') <> 'aal2' then 'revoked'
 else 'active' end)
 ...
 left join identity.sessions a on a.id=s.auth_session_id and a.user_id=s.actor_user_id
 where s.actor_user_id=fortis.current_user_id()
 and s.auth_session_id=fortis.current_session_id() and s.ended_at is null
 limit 1;
$f$;
...
returns text language sql stable security definer set search_path = public as $f$
 ...
 else (select role from public.user_organizations where user_id=fortis.current_user_id() and organization_id=p_org and revoked_at is null limit 1) end
...
begin
 perform 1 from identity.sessions where id=p_session and user_id=p_actor and (not_after is null or not_after>now()) for update;
 ...
 and (not (p.mfa_required or exists(select 1 from identity.mfa_factors f where f.user_id=p_actor and f.status='verified'))
 or exists(select 1 from identity.sessions a where a.id=p_session and a.aal='aal2'));
...
begin
  ...
  update public.conversations
     set assigned_to_user_id = p_to_user_id,
         ...
         -- dono nenhum. Lido de identity.users porque quem chama esta função
         -- (RPC) não necessariamente tem acesso ao Admin API — a definer
         -- resolve por dentro.
         assigned_to_user_name = case
           when p_to_user_id is null then null
           else (select profile ->> 'full_name' from identity.users where id = p_to_user_id)
         end,
  ...
  values
    (p_organization_id, p_conversation_id, v_from, p_to_user_id, fortis.current_user_id(), p_reason);
...
begin
  ...
  --     policies de `merge_queue`. Sessão de service role (fortis.current_user_id() nulo) não
  ...
  select p_organization_id, l.id, p_contato_principal, 'crm', p_contato_principal,
         ...
         '{}'::jsonb, now(), fortis.current_user_id()
