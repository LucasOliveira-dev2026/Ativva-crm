-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260907010000_0225_google_reconciliacao.sql. Do not edit.
...
begin
 ...
 if tg_op='INSERT' then
  ...
  if fortis.current_user_id() is not null then
 ...
 decision:=fortis.current_user_id()=old.owner_user_id and public.fn_role_at_least(new.organization_id,'agent') and public.fn_support_write_allowed(new.organization_id)
  and old.google_conflict is not null and new.google_conflict-'resolution'=old.google_conflict-'resolution'
  and new.google_conflict->'resolution'->>'actor_id'=fortis.current_user_id()::text
 ...
 if fortis.current_user_id() is not null and (row(new.google_synced_at,new.google_sync_error) is distinct from row(old.google_synced_at,old.google_sync_error)
  or (new.google_next_attempt_at is distinct from old.google_next_attempt_at and not coalesce(fortis.current_user_id()=old.owner_user_id and public.fn_role_at_least(new.organization_id,'agent') and public.fn_support_write_allowed(new.organization_id)
 ...
 if fortis.current_user_id() is not null and ((new.google_conflict is distinct from old.google_conflict and not coalesce(decision,false)) or row(new.google_base_projection,new.google_pending_write,new.google_claim_token,new.google_claim_epoch,new.google_claim_until,new.google_synced_local_revision,new.google_etag,new.google_connection_id,new.google_calendar_id,new.google_event_id)
 ...
 inbound:=row(new.title,new.description,new.location_kind,new.location_details,new.guest_email) is not distinct from row(old.title,old.description,old.location_kind,old.location_details,old.guest_email) and fortis.current_user_id() is null and new.google_base_projection is distinct from old.google_base_projection
...
begin
 if p_remote and (fortis.current_user_id() is not null or (p_patch-'starts_at'-'ends_at'-'time_zone'-'status'-'cancellation_reason')<>'{}'::jsonb or coalesce(p_patch->>'status','cancelled')<>'cancelled') then raise exception 'google_patch_forbidden' using errcode='42501';end if;
 if fortis.current_user_id() is not null and (not public.fn_role_at_least(p_org,'agent') or not public.fn_support_write_allowed(p_org)) then raise exception 'appointment_forbidden' using errcode='42501'; end if;
...
returns void language plpgsql security definer set search_path=public as $$
declare actor uuid:=fortis.current_user_id(); expected jsonb; actual jsonb;
begin
...
returns boolean language sql stable security definer set search_path=public as $$
 ...
 select (fortis.current_user_id() is null or p_org in (select public.fn_user_org_ids())) and not exists(
...
begin
 if fortis.current_user_id() is null or not public.fn_role_at_least(p_org,'agent') or not public.fn_support_write_allowed(p_org) then raise exception 'google_resolution_forbidden' using errcode='42501';end if;
 select contact_id into contact from public.calendar_appointments where organization_id=p_org and id=p_id;
 if contact is not null then perform public.fn_service_lock(p_org,contact);end if;
 select * into a from public.calendar_appointments where organization_id=p_org and id=p_id for update;
 if not found or a.owner_user_id is distinct from fortis.current_user_id() then raise exception 'google_resolution_forbidden' using errcode='42501';end if;
 ...
 if p_choice='retry' then
  ...
  -- e as revisões não mudam, actor_id é fortis.current_user_id(), não input do browser.
  update public.calendar_appointments set google_conflict=google_conflict||jsonb_build_object('resolution',jsonb_build_object('choice',p_choice,'actor_id',fortis.current_user_id())),google_next_attempt_at=now()
...
returns boolean language sql stable security definer set search_path=public as $$
 select case when fortis.current_user_id() is not null and p_org not in(select public.fn_user_org_ids()) then true else exists(
