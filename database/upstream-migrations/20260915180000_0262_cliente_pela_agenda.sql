-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260915180000_0262_cliente_pela_agenda.sql. Do not edit.
...
-- isso; o admin client (service role, `fortis.current_user_id()` nulo) continua passando.
--
...
begin
  ...
  -- A ESCRITA SE ANUNCIA. `fortis.current_user_id()` continua preenchido aqui dentro — uma
...
--
--     `fortis.current_user_id() is null` PASSA de propósito: é o admin client (service role),
...
-- fora do schema exposto. Ela existe porque `fortis.current_user_id()` continua preenchido
...
-- SESSÃO: `fortis.current_user_id()` é o que permite conferir papel aqui dentro. Nunca pelo
