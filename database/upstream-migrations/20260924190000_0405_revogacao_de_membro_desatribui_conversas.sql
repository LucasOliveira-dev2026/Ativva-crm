-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260924190000_0405_revogacao_de_membro_desatribui_conversas.sql. Do not edit.
...
begin
 if new.revoked_at is not null or new.role not in('agent','manager','admin') then
  ...
  loop
    ...
    values
      (new.organization_id,v_conv.id,new.user_id,null,fortis.current_user_id(),'member_revoked');
  end loop;
 end if;
