-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260717120000_0031_conversation_assignment_events.sql. Do not edit.
...
-- fortis.current_user_id() (null = worker/sistema via service role — não confia em input do
...
create table if not exists public.conversation_assignment_events (
  ...
  from_user_id    uuid references identity.users(id) on delete set null, -- null = sem dono / com a IA
  to_user_id      uuid references identity.users(id) on delete set null, -- null = liberada (volta à fila/IA)
  changed_by      uuid references identity.users(id) on delete set null, -- null = sistema (worker de routing / agente IA)
...
revoke all on public.conversation_assignment_events from crm_anonymous;
