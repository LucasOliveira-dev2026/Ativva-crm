-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260906120000_0224_presenca_e_recuperacao.sql. Do not edit.
...
alter table public.calendar_appointments
 ...
 add column if not exists outcome_user_id uuid references identity.users(id) on delete set null,
...
begin
 ...
 if new.status is distinct from old.status and new.status in ('completed','no_show') then
  ...
  new.outcome_user_id:=case when new.outcome_user_id is null and not exists(select 1 from identity.users where id=old.outcome_user_id) then null else old.outcome_user_id end;
...
declare a record;
begin
 if fortis.current_user_id() is null and old.service_revision is null and new.service_revision is not null and new.direction='inbound' then
...
begin
 if tg_table_name='job_queue' then
  if fortis.current_user_id() is not null and ((tg_op<>'DELETE' and new.kind='followup_turn') or (tg_op<>'INSERT' and old.kind='followup_turn')) then
 ...
 elsif fortis.current_user_id() is not null and ((tg_op<>'DELETE' and new.idempotency_key ~ ':[0-9]+$') or (tg_op<>'INSERT' and old.idempotency_key ~ ':[0-9]+$')) then
...
begin
  ...
  if fortis.current_user_id() is not null and p_event_type in ('message.received','appointment.outcome_confirmed') then
  ...
  if fortis.current_user_id() is not null and (
  ...
  if v_org_id is null then
    select organization_id into v_org_id
      ...
      where user_id = fortis.current_user_id() and revoked_at is null
      limit 1;
  end if;
  if v_org_id is null then
  ...
  if fortis.current_user_id() is not null
