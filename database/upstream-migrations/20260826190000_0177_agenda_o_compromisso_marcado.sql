-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260826190000_0177_agenda_o_compromisso_marcado.sql. Do not edit.
-- ============================================================================
...
create table if not exists public.calendar_event_types (
  ...
  default_owner_user_id uuid references identity.users(id) on delete set null,
...
-- e não em identity.users: quem trabalha em duas organizações pode ser verde numa
...
revoke all on public.calendar_event_types from crm_anonymous;
...
revoke all on public.calendar_appointments from crm_anonymous;
...
create policy calendar_availability_exceptions_write on public.calendar_availability_exceptions
  using (
    public.fn_is_platform_admin()
    or ((organization_id in (select public.fn_user_org_ids()))
        and (user_id = fortis.current_user_id() or public.fn_role_at_least(organization_id, 'manager')))
  )
  with check (
    public.fn_is_platform_admin()
    or ((organization_id in (select public.fn_user_org_ids()))
...
revoke all on public.calendar_availability_exceptions from crm_anonymous;
        ...
        and exists (
          select 1 from public.calendar_connections c
           where c.id = connection_id
             and (c.user_id = fortis.current_user_id()
                  or public.fn_role_at_least(c.organization_id, 'manager'))
        ))
  );
revoke all on public.calendar_connection_calendars from crm_anonymous;
...
revoke all on public.calendar_external_events from crm_anonymous;
...
revoke all on public.calendar_connections from crm_anonymous;
