-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260906010000_0221_interface_por_vinculo.sql. Do not edit.
...
do $$ begin
 ...
 if exists(select 1 from pg_publication where pubname='crm_realtime') and not exists(
  select 1 from pg_publication_tables where pubname='crm_realtime' and schemaname='public' and tablename='user_organizations') then
  alter publication crm_realtime add table public.user_organizations;
 end if;
end $$;
...
begin
  ...
  insert into public.user_organizations(organization_id, user_id, role, accepted_at, interface_settings)
    values (org.id, p_actor, 'admin', now(), case when lower(p_request->>'owner_email') =
      (select lower(email) from identity.users where id = p_actor)
