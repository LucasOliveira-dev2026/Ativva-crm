-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260908140000_0235_voz_isolamento_lgpd_e_dono.sql. Do not edit.
...
alter table public.voice_calls
  add column if not exists owner_user_id uuid references identity.users(id) on delete set null;

comment on column public.voice_calls.owner_user_id is
  'Quem esteve NA LINHA. Gravado pela rota de atender/iniciar e confirmado pelo campo `owner` do upstream (que é o X-Client-Id que nós mandamos, ou seja, o identity.users.id). Distinto de created_by, que só existe na chamada iniciada pelo CRM e é nulo em toda ligação recebida.';
...
revoke all on public.voice_calls from crm_anonymous;
