-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260828000000_0202_o_nome_do_atendente_custava_uma_chamada_http.sql. Do not edit.
...
-- 2. Backfill das linhas já atribuídas, lendo `identity.users.profile
...
--    subquery contra `identity.users` no mesmo UPDATE que grava o id.
--
...
comment on column public.conversations.assigned_to_user_name is
  'Cópia do nome de quem atende (identity.users.profile->>''full_name''), escrita por fn_conversation_assign no mesmo UPDATE que grava assigned_to_user_id, e zerada junto quando a atribuição é removida. Existe para evitar 1 chamada HTTP ao GoTrue Admin API por atendente único na listagem do Inbox — ver lib/users/nome-do-atendente.ts. NULL quando a conversa não está atribuída, ou quando o atendente não tem full_name em user_metadata.';
...
update public.conversations c
   set assigned_to_user_name = u.profile ->> 'full_name'
...
begin
  ...
  update public.conversations
     set assigned_to_user_id = p_to_user_id,
         ...
         -- nenhum. Lido de identity.users porque quem chama esta função (RPC) não
