-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260722140000_0060_message_templates.sql. Do not edit.
...
create policy "message_templates_select" on message_templates
  for select using (
    (
      ...
      and (owner_user_id is null or owner_user_id = fortis.current_user_id())
    )
...
create policy "message_templates_write" on message_templates
  for all using (
    organization_id in (select fn_user_org_ids())
    and (
      (owner_user_id = fortis.current_user_id() and fn_role_at_least(organization_id, 'agent'))
