-- GENERATED by scripts/fortis/baseline/build.mjs from supabase/migrations/20260907030000_0227_autonomia_e_respostas_revisadas.sql. Do not edit.
...
create table if not exists public.ai_reply_drafts(
 ...
 approved_by uuid references identity.users(id),approved_at timestamptz,approved_support_session_id uuid references public.platform_support_sessions(id),send_job_id uuid unique references public.job_queue(id),message_id uuid references public.messages(id),
...
begin
 if fortis.current_user_id() is null or not public.fn_role_at_least(p_org,'agent') or not public.fn_support_write_allowed(p_org) or not public.fn_session_mfa_proven() then raise exception 'reply_forbidden' using errcode='42501';end if;
 ...
 if d.status in('approved','sending','sent') and p_action='approve' and d.approved_by=fortis.current_user_id() and d.approved_body=p_body then return d.send_job_id;end if;
 ...
 update public.ai_reply_drafts set status='approved',edited_body=p_body,approved_body=p_body,approved_by=fortis.current_user_id(),approved_at=now(),approved_support_session_id=case when public.fn_support_context()->>'organization_id'=p_org::text then (public.fn_support_context()->>'id')::uuid else null end,send_job_id=jid,
...
returns jsonb language sql stable security definer set search_path=public as $$
 ...
 left join identity.sessions au on au.id=ss.auth_session_id and au.user_id=ss.actor_user_id and (au.not_after is null or au.not_after>now())
